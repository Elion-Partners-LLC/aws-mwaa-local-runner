...
from elion.utils import trim_none_values
...
class AwsConnectionWrapper(LoggingMixin):
    ...
    def conn_repr(self):
        return (
            f"AWS Connection (conn_id={self.conn_id!r}, conn_type={self.conn_type!r})"
        )
    ...
    def __post_init__(self, conn: Connection):
        ...
        if (
            "profile" in extra
            and "s3_config_file" not in extra
    ...
    def _get_credentials(
        ...
        **kwargs,
        ...
        if self.login and self.password:
            ...
            self.log.info(
                "%s credentials retrieved from extra['s3_config_file']", self.conn_repr
            )
    ...
    def _get_assume_role_configs(
        ...
        **kwargs,
        ...
        self.log.debug(
            "Retrieve assume_role_method=%r from %s.",
            assume_role_method,
            self.conn_repr,
        )
